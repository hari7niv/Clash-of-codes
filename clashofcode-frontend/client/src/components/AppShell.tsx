...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell text-[#f3f0ea] selection:bg-[#f04432] selection:text-white">
    ...
    <aside className={`arena-rail ${mobileOpen ? "is-open" : ""}`}>
      ...
      <div className="mt-auto px-4 pb-5"><Link href="/profile" onClick={closeMobile} className={`profile-rail ${location.startsWith("/profile") ? "is-active" : ""}`}><Avatar initials={player.initials} tone="red" size="sm" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium text-[#eeeef0]">{player.handle}</span><span className="block font-mono text-[9px] text-[#747783]">{player.rating} RATING</span></span><RankBadge rank={player.rank} className="scale-90 origin-right" /></Link></div>
    </aside>
    {mobileOpen && <button aria-label="Close navigation" className="fixed inset-0 z-30 bg-black/60 lg:hidden" onClick={closeMobile} />}
    <main className="app-canvas"><div className="desktop-topline"><div className="flex min-w-0 items-center gap-3"><span className="section-kicker">Competitive desk</span><span className="topline-divider" /><span className="topline-live"><span className="live-dot" /> Arena live</span><span className="font-mono text-[9px] tracking-[.13em] text-[#747783]">SEASON 03 / WEEK 07</span></div><div className="ml-auto flex items-center gap-2"><Link href="/rooms/create" className="topline-action"><Plus className="h-3.5 w-3.5" />Create room</Link><button className="icon-button h-8 w-8" aria-label="Notifications"><Bell className="h-4 w-4" /><span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-[#f04432]" /></button><span className="topline-run"><Flame className="h-3 w-3 text-[#e1a759]" />{player.streak} day run</span></div></div><div className="app-workspace">{children}</div></main>
  </div>;
}
