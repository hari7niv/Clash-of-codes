/**
 * Style system: Tournament Console — practice is a tactical training desk that turns weakness
 * signals into a clear next exercise, rather than reading like a generic lesson catalogue.
 */
...
export default function Practice() {
  ...
  useEffect(() => {
    api.get("/problems/daily").then(res => setDailyProblem(res.data)).catch(console.error);
  }, []);
  ...
  const handleStartDrill = (id: string) => {
    if (id) setLocation(`/practice/${id}`);
  };

  const handleDailyChallenge = () => {
    if (dailyProblem) setLocation(`/practice/${dailyProblem.id}`);
  };
