...
export default function RoomCreate() {
  ...
  const [activeGuestLink, setActiveGuestLink] = useState("");
  const [submitting, setSubmitting] = useState(false);

  const { createRoom } = useRoomData(null);
  ...
  const shareInvitation = async () => { if (navigator.share) { await navigator.share({ title: "Join my ClashOfCode room", text: `Join my coding room with code ${activeRoomCode}.`, url: activeGuestLink }); toast.success("Share sheet opened"); } else copy(activeGuestLink, "Guest link"); };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (response && response.roomCode) {
        ...
        setActiveGuestLink(response.guestLink || `${window.location.origin}/join/${response.roomCode}`);
        setCreated(true);
      } else {
        toast.error("Failed to create room: invalid response");
      }
    } catch (err: any) {
